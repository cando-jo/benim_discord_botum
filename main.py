...
@bot.command()
async def lol_resimleri(ctx):
    resimler_listesi = os.listdir('lol_resimleri') #['resim1.jpeg', 'resim2.jpeg', 'resim3.jpeg']
    with open(f'lol_resimleri/{random.choice(resimler_listesi)}', 'rb') as f:
        # Dönüştürülen Discord kütüphane dosyasını bu değişkende saklayalım!
        picture = discord.File(f)
   # Daha sonra bu dosyayı bir parametre olarak gönderebiliriz!
    await ctx.send(file=picture)
